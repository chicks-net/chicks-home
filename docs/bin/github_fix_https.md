...
Convert the current repo's `origin` remote from HTTPS to SSH so Git stops
...
extracts the user and repo slugs, and runs `git remote set-url` to switch
...
No flags, no arguments. Run it from inside the repo you want to fix. If
